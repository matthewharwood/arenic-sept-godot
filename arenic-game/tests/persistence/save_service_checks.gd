...
func _run() -> void:
	...
	_check(result.payload.run.intro_step == 0, "New games durably begin at the opening quote")
	...
	_check(root.get_node("RunSetup").intro_step == 0 and shell.hero.cell == Vector2i(33, 15), "Class confirmation preserves the quote and centers the new founder")
	...
	_check(not ArenicSaveMigrations.upgrade({"schema_version": 1}, 2).ok, "Malformed legacy migration fails closed")
	...
	_check(run.intro_step == 6, "Established development guilds skip the prologue")
