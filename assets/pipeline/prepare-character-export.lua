-- Configure saved character sources without saving artwork or exporting textures/data.
...
local function repository_from(path)
  ...
  while folder ~= '' do
    if app.fs.isFile(app.fs.joinPath(folder, 'arenic-game', 'project.godot'))
        and app.fs.isDirectory(app.fs.joinPath(folder, 'assets', 'characters')) then
      return folder
    end
  ...
  error('Expected character sources beneath assets/characters beside arenic-game.')
end
...
if not selected and not all then
  assert(active and active.hasAssociatedFile, 'Open a saved character .aseprite source first.')
  selected = active.filename
end
local root = repository_from(absolute(app.params.root or selected or app.fs.currentPath))
local source_root = app.fs.joinPath(root, 'assets', 'characters')
local output_root = app.fs.joinPath(root, 'arenic-game', 'assets', 'characters')
...
assert(#sources > 0, 'No saved character .aseprite sources found.')
