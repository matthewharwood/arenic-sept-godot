...
The [main-game HUD](hud.md) shows forty roster positions, the selected hero and vitals, four fixed ability slots, the arena map, navigation hints, and overview/zoom controls. Slot 1 uses the chosen starter ability with its existing cooldown or active state. Its full-width damage bar and phase label read the selected arena’s cumulative combat ledger, not enemy health. Loot, auction, craft, additional roster acquisition, and recording remain future work. The shell owns input and simulation; the HUD displays their state.
