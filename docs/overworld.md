...
**Implemented:** New class selection begins the [Guild House introduction](introduction.md) with one chosen hero at tile `(33, 15)` in close view. Established runs can enter overview with Guild House highlighted and no hero actively selected. Native 19 × 19 hero sprites can be selected, moved, and use their starter abilities in a focused arena. Nine arenas each render a complete 66 × 31 tile lattice and contain an immortal combat target: eight matching bosses and Guild House’s existing training target, now presented as the clearing’s tavern. The persistent HUD shows real cumulative damage and phase progress for the selected arena. [Starter combat](combat.md), [guild recruitment](guild.md), and [recording](recording.md) document the implemented gameplay and run-state persistence.
...
Each arena resource exposes its identity, slot, hotkey, `visual_theme`, `music`, `boss`, `boss_origin_cell`, `boss_facing`, `boss_combat_size`, `boss_visual_offset`, `phase_damage`, `training_target_frames`, and optional `content_scene`. Eight definitions reference matching resources in `arenic-game/data/bosses/`; Guild House leaves `boss` null and assigns `tavern_frames.tres` through `training_target_frames`. `boss_origin_cell` defaults to `(30, 22)`, and `boss_facing` to `"n"`. The explicit gameplay footprint defaults to 6 × 6 independently of artwork; the absolute phase threshold defaults to 20 damage.
...
Each arena's `Tiles` node is an `ArenicArenaTiles` / `MultiMeshInstance3D` with **2,046 real PlaneMesh instances** on XZ. Each plane measures 0.25 × 0.25 units. Arenas own separate instance transforms and theme material overrides while sharing the immutable mesh and fallback texture. The themed floor shader preserves the 19 × 19 pixel cell raster. Eight arena floors add one center dot at `(9, 9)` with decorative seams and ornaments; Guild House samples four native grass variants without center dots, architectural seams or a perimeter frame. Its earth paths are separate transparent overlays. The fallback is a white 19 × 19 texture with one gray center pixel. `tile_center(cell)` returns the local center matching the grid conversion above.

`ArenicArenaView` mounts the assigned boss as an unshaded, nearest-filtered `AnimatedSprite3D` playing `idle_n` by default. Its 114 × 114 frame uses `pixel_size = 0.25 / 19` and the centered `(57, 57)` pivot, so the art canvas covers exactly six cells in each direction. It sits 0.01 units above the tiles; its center is 2.5 tile steps toward +X/−Z from `boss_origin_cell`, plus any authored `boss_visual_offset`. The full canvas must fit inside the arena. Guild House presents its existing training target as a native 247 × 171 tavern in an outdoor grass clearing, with clustered trees, winding earth paths, two wood sources, two gold mines and the Keeper seated outside at `(33, 18)`. His portrait and dialogue portrait remain unchanged. [guild_clearing.tres](../arenic-game/data/world/guild_clearing.tres) exposes scenery placements and visual variants in the Inspector; gathering locations and rules remain in their own resource. The tavern's four idle names share one static native frame, while the seated Keeper has four idle and four beckoning frames. These visual changes preserve combat identity and the explicit `boss_combat_size` used by targeting and movement blocking. All nine targets remain immortal; accumulated damage advances available boss appearances without removing the target. See the [clearing art booklet](../assets/environment/guild_clearing/README.md), [combat rules](combat.md) and [boss source contract](../assets/bosses/README.md).

## Arena themes

All nine arena definitions retain their source-matched palette. Flat backdrop/floor/foreground layers and 119 bounded under-floor motes add visual identity. Eight arenas retain 96 fixed native border props; Guild House replaces its twelve indoor decorations with 53 authored trees and bounded earth-path stamps, alongside the tavern and gathering-site art. Actors retain their source colors and movement/camera ownership remains unchanged. The [arena theme guide](arena-themes.md) documents palette mappings, OKLCH conversion, source/runtime assets and layer ordering. Its earlier validation records and the screenshots below predate the 2026-09-13 outdoor-clearing restyle.
...
The [main-game HUD](hud.md) shows forty roster positions, the selected hero and vitals, four fixed ability slots, the arena map, navigation hints, and overview/zoom controls. Slot 1 uses the chosen starter ability with its existing cooldown or active state. Its full-width damage bar reads the selected arena’s cumulative combat ledger. The top strip pairs the arena name with actual boss conditions; recruitment stays beside the roster, and Overview/Save & Title live in the H menu. Guild recruitment and recording use the existing game models. The shell owns input and simulation; the HUD displays their state.

Overview replaces the four attack buttons and Record with five navigation
actions. Each opens the corresponding empty tab in the shared tools overlay;
switching tabs does not create gameplay state. The overlay owns keyboard and
pointer input while open, and closes before an actual gameplay decision opens.
Combat, recorded ghosts, encounter/music clocks, and autosave keep running.
Continue reconstructs the overlay closed; it is separate from saved recording
...
| Tab / Shift+Tab | Select the viewed arena’s remembered hero, then cycle its members in roster order. An empty arena stays unselected. |
...
| Enter | Zoom to the selected arena without selecting a hero. |
...
Hero movement follows the reference's [tile-step input](https://github.com/matthewharwood/arenic/blob/60da21575de191461a12f2b2f68a7efd1b254bcd/crates/arenic_game/src/grid.rs#L51-L102) and [edge-walking](https://github.com/matthewharwood/arenic/blob/60da21575de191461a12f2b2f68a7efd1b254bcd/crates/arenic/src/travel.rs#L144-L189): a new arrow press moves exactly one tile, holding never repeats, simultaneous directions combine into a diagonal, and opposites cancel. Crossing an arena edge enters its neighbor at the opposite edge and moves camera focus with the hero. Horizontal crossings take priority at corners; the outer world boundary clamps and never wraps. The shell rejects steps into registered enemy gameplay footprints, including Guild House’s unchanged training-target footprint beneath the tavern presentation; transparent artwork does not determine collision.

The Godot port deliberately limits hero control to its focused arena so overview arrows retain map navigation. Camera hotkeys do not teleport the hero. Tab selects only heroes in the viewed arena. Clicking the hero is an addition to the original puck controls. Clicking empty floor deselects; arrows then leave both hero and camera still until selection resumes. Letter shortcuts, brackets, P, and Escape remain available. Escape returns to overview, not the title.

Entering overview clears every hero's active selection and the blue selected
roster fill. The run retains its remembered hero and each arena's remembered
member, including when overview navigation adopts another arena's member.
Returning through ordinary zoom leaves heroes unselected: the first Tab focuses
the remembered hero, and the next Tab cycles. Clicking a roster member explicitly
zooms to that member and selects it. Stage replacement also enters an unselected
overview. Continue restores exact selection in close view and normalizes older
overview saves to this same unselected state.

## Hero state, sprite, and input ownership

`RunSetup.choose_class()` creates one `ArenicHeroState`, with the chosen class definition, `arena_id = "guild_house"`, cell `(33, 15)` for a new introduction (the existing `(30, 15)` spawn otherwise), north facing, and selected status. `begin_new_game()` resets both hero and combat state. A direct GameShell launch supplies the Hunter as a development default. The hero state lives in the run autoload; replacing a stage remounts a new view without resetting position or identity. A replacement world must contain the hero's current arena; an incompatible stage is rejected before the existing stage is removed.
...
`GameShell.entry_sequence` is an optional `PackedScene` for additional authored sequences. New guilds first enter the [Guild House introduction](introduction.md): a quote, Keeper dialogue and gate unlock in close view. Its saved lock owns ordinary navigation until completion. Existing guilds continue their saved view. Optional entry sequences do not bypass the introduction.
...
A native Godot Play launch before the 2026-09-13 outdoor-clearing restyle was visually tested through title → Warrior choice → Guild House. Mouse deselection prevented movement; clicking the sprite reselected it, and physical arrow inputs changed its tile and facing. The historical screenshot below shows that actual 1280 × 720 viewport. Its 19 × 19 Warrior canvas matched all 143 opaque pixels of the paused east-facing source frame exactly, with no scaling or filtering mismatch. Runtime error log was empty. Animation playback was restored after capture.
...
### Arena-scoped control

Every arena navigation path adopts that arena’s remembered member or its first resident. Empty arenas clear active selection; retained identity memory cannot receive movement, casts, ghost-breakout prompts, or recording commands from another arena. Tab never redirects from an empty arena to a remote hero. Returning to a populated arena restores its own memory.

The first recruit in an empty Guild House becomes that arena’s default and is selected immediately when its close view is active. Later recruits preserve its existing selection. The same rule covers an unselected member respawning into the viewed empty Guild House; controlled movement into any arena remembers the arriving hero. Overview retains its existing explicit-selection behavior. Held inputs are cleared on navigation and selection changes.
