...
hydrate prepared gameplay at 640 × 360, then verify the actual requested framebuffer
...
Most gameplay browser cases start from canonical per-class saves generated by
`tests/web/prepare-fixtures.gd` during the disposable probe build. It uses real
run constructors and the shipping document/codec validation with every autoload
disabled, so generation cannot access native saves. A private static setup page
inserts the document before WASM starts; real IndexedDB hydration, Continue and
the slot picker load it. Explicit introduction tests and the clean production
smoke retain actual new-game onboarding. Fixtures and their setup page never
enter the production site, and no gameplay clock or save schema changes.

Timing rules run in native simulations that call the real model or shell with
whole ticks directly. Ability ownership covers every ordered starter pair and
320 concurrent heroes; gathering covers both complete recorded resource routes
and their next-loop deposits, including exact fill ticks; rewind checks simulate
full histories and countdown boundaries. None waits for the simulated duration.
The browser gathering case keeps one visible wood route, actual recording input,
bag animation and ghost replay; native checks own the repeated wood/gold rules.
The browser rewind case observes 300 ticks rather than 900 while retaining actual
reverse movement and 3–2–1 input blocking. Rendering, decoded audio, pointer/input
and storage reload still require real browser execution; the full release gate
is not instantaneous. Reloads wait for fresh engine startup and the HTML loader
to release the canvas before clicking title controls.
